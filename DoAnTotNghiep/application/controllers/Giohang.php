...
class Giohang extends CI_Controller {
    ...
    public function index(){
        $this->data['title']='Siêu thị Thiên Nhiên - Giỏ hàng của bạn';
    ...
    public function info_order(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            $this->data['title']='Siêu thị Thiên Nhiên - Thông tin đơn hàng';
    ...
    public function thankyou(){
        if($this->session->userdata('info-customer')||$this->session->userdata('sessionKhachHang')){
            ...
            $config['smtp_user']    = 'sieuthithienhien.vn@gmail.com';
            ...
            $this->email->from('Sieuthithiennhien.vn@gmail.com', 'Siêu thị Thiên Nhiên');
            $list = array($val['email']);
            $this->email->to($list);
            $this->email->subject('Hệ thống Siêu thị Thiên Nhiên');
        ...
        $this->data['title']='Siêu thị Thiên Nhiên - Kết quả đơn hàng';
...
public function checkproduct()
    {
        ...
        if($this->request->getMethod() == 'post'){
            ...
            $price_ship = $this->request->getvar('price_ship');
            $coupon = $this->request->getvar('coupon');
            $province = $this->request->getvar('province');
            $district = $this->request->getvar('district');
            $address = $this->request->getvar('address');
            
            $trash = $this->request->getvar('trash');
            // $total_prices = $this->request->getvar('total-prices');
            // $sum_prices = $this->request->getvar('sum-prices');
            foreach($productname as $index => $value){

                $product_i4 = $Mproduct->getProductById($productname[$index]);
                if($quantity[$index] >  $product_i4['number']){
                    $data['message'] = 'not enough';
                    $data['notify'] = 'Sản phẩm '.$product_i4['name'].' không còn đủ số lượng';
                    ...
                    $data_invoice = [
                        ...
                        // 'orderdate'=> $_SESSION['customer']['orderdate'],
                        'orderdate' => date('Y-m-d H:i:s'),
                        'status' => "0",
                    ];
                    $id = $Morders->insert($data_orders);
                    foreach($productname as $index => $value){
                        $data_orderDetail = [
                            'order_id' => $id,
                        ...
                        $product_i4 = $Mproduct->getProductById($productname[$index]);
                        $data_product = [
                            'number'=> $product_i4['number'] - $quantity[$index],
                        ];
                        // var_dump($data_product);
                        // die();
                        $Mproduct->update($productname[$index],$data_product);
                        // var_dump($data_invoiceDetail);
                        $orderDetail = $Morderdetail->insert($data_orderDetail);
                    }
                    if($orderDetail > 0){
                        $data['message'] = 'success';
                        return view('frontend/giohang/info-order',$data);
                    } else{
                        $data['message'] = 'fail';
                        return view('frontend/giohang/info-order',$data);
                    }
                }
                return view('frontend/giohang/index',$data);
            }
