										...
										<tr>
											...
											<td>
												<div class="quantity clearfix">
													<input name="quantity" id="<?php echo $row['id'] ?>" class="form-control" type="number" value="<?php echo $value ?>" min="1" max="<?= $row['number'] ?>" onchange="onChangeSL(<?php echo $row['id'] ?>)">
													
													<?php if($value> $row['number']): ?>
														<p>Nhập số lượng nhỏ hơn <?php echo $row['number'] ?></p>
													<?php endif?>
												</div>
											</td>
											<td>
							...
							<div class="clearfix btn-submit" style="padding-left: 10px;margin-top: 20px;">
								<table class="table total-price" style="border: 1px solid #ececec;">
									<tbody>
										...
										<tr>
											<td colspan="2"><h4>ĐỔI TRẢ DỄ DÀNG</h4>
										    <p style ="font-size: 15px">(Đổi trả trong vòng 330 ngày)</p></td>
										</tr>
										<tr>
											<td colspan="2"><h4>TỔNG ĐÀI BÁN HÀNG 0241.366.244</h4>
										    <p style ="font-size: 15px">(Miễn phí từ 8h00 - 21h00 hàng ngày)</p></td>
										</tr>
